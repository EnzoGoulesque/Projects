    ...
    <div class="center">
            ...
            if(($tournoiouvert)){
                for($tournoiouvert; $tournoiID != 0; $tournoiID = $tournoiID - 1 ){
                    ...
                    $c = $db->prepare("SELECT * FROM tournament_t WHERE tourneyId = :tourneyId AND stateT = 4 ORDER BY tourneyId DESC"); //Crée un fetch avec les informations de l'utilisateur
					...
					$zz->execute(['tourneyId'=> $tournoiID,'bracket'=> 1 ]);
        			$Number = $zz->rowCount();
                    ...
                    if($tournoiouvert2){
                        ...
                        include 'include/SeeTourneyFinished.php';
                        echo "</div>";
                    }
